...
const PATHS: Record<StampIconName, React.ReactNode> = {
...
export interface StampIconProps extends React.SVGAttributes<SVGSVGElement> {
  name: StampIconName;
  size?: number;
  animated?: boolean;
}

export function StampIcon({ name, size = 20, animated = true, ...props }: StampIconProps) {
  return (
    <svg
      width={size}
      height={size}
    ...
    >
      {animated ? (
        <style>{`@keyframes stampDraw { from { stroke-dashoffset: 1; } to { stroke-dashoffset: 0; } } @media (prefers-reduced-motion: no-preference) { .stamp-draw { stroke-dasharray: 1; animation: stampDraw 0.55s ease-out backwards; } }`}</style>
      ) : null}
      <g pathLength={1} className={animated ? "stamp-draw" : undefined}>
        {PATHS[name]}
      </g>
    </svg>
  );
