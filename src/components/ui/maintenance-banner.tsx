...
export function MaintenanceBanner({ message = "Scheduled maintenance in progress. Services may experience brief interruptions.", startsInMinutes, onDismiss, className, ...props }: MaintenanceBannerProps) {
  ...
  return (
    <div className={cn("relative flex items-center gap-3 border-b border-warning/40 bg-warning/10 px-4 py-2", className)} role="status" {...props}>
      <span className="flex size-5 shrink-0 items-center justify-center rounded-full bg-warning/20 text-warning animate-pulse motion-reduce:animate-none" aria-hidden="true">
